...
class Generator(ModuleBase):
    def __init__(self, img_shape,gf,AU_num,num_layers=4,f_size=6,tranform_layer=False,res_blocks=1):
        ...
        ### ENCODER
        layers = [
        conv2d(channels, gf, f_size=4,stride=2),
            conv2d(gf, gf*2,f_size=4),
            conv2d(gf*2, gf*4,f_size=4),
            conv2d(gf*4, gf*8,f_size=4)
        ]
        ...
        ### DECODER
        layers = [
            deconv2d(gf*8, gf*4, f_size=4), ## residual block
            deconv2d(gf*8, gf*2, f_size=4), 
            deconv2d(gf*2, gf, f_size=4,output_padding=1),    
            deconv2d(gf, channels,f_size=4,stride=2,output_padding=0)
        ]
    ...
    def encode(self, x):
        ...
        for layer in self.enc_layers:
            z = layer(z)
            zs.append(z)
        #print("last z:",z.shape)
        return zs
    
    def translate_decode_old(self, z,au_target):
    ...
    def translate_decode(self, z,au_target):
        _au_target = au_target.view(au_target.size(0), -1, 1, 1).repeat(1, 1, 5, 5)
        _z = z[-1]
        #print("_au_target",_au_target.shape)
        #print("_z",_z.shape,len(z))
        res_block = torch.cat([_z, _au_target], dim=1)
        #print("- before 1x1 res_block",res_block.shape)
        res_block = self.conv_1_1(res_block)
        #print("- after 1x1 res_block",res_block.shape)
        for i,layer in enumerate(self.dec_layers):
            res_block = layer(res_block)
            if len(z) > 0 and i < self.res_blocks:
                _z = z[-2-i]
                #print(i,"res_block",res_block.shape)
                #print(i,"_z",_z.shape,len(z))
                res_block = torch.cat([res_block, _z], dim=1)
            #print(i," -- final -- res_block",res_block.shape)
        return res_block
